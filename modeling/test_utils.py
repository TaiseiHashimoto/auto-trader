...
def test_compute_macd(mocker):
    ...
    assert compute_ema.call_args_list == [
        mocker.call(s, ema_window_size_short),
        mocker.call(s, ema_window_size_long),
    ]
    assert (
        compute_sma.call_count == 1
        and compute_sma.call_args_list[0][0][1] == sma_window_size
    )
...
def test_compute_stochastics():
    ...
    expected_k = pd.Series(
        [np.nan, np.nan, 2 / 4, 1 / 2, 4 / 4, 1 / 3, 2 / 2, 5 / 5, 2 / 3, 0 / 6],
        dtype=np.float32,
    )
...
def test_compute_ctirical_idxs():
    actual_critical_idxs, actual_critical_switch_idxs = utils.compute_critical_idxs(
        values=np.array([1.0, 2.0, 3.0, 0.0, 2.0, 1.0, 5.0, 3.0, 1.0]), thresh_hold=1.5
    )
    ...
    actual_critical_idxs, actual_critical_switch_idxs = utils.compute_critical_idxs(
        values=np.array([3.0, 2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.9, 0.8]), thresh_hold=1.5
    )
...
def test_compute_critical_info():
    values = np.array([1.0, 2.0, 3.0, 0.0, 2.0, 1.0, 5.0, 3.0, 1.0])
    ...
    values = np.array([3.0, 2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.9, 0.8])
...
def test_create_critical_labels():
    values = np.array([1.0, 2.0, 3.0, 0.0, 2.0, 1.0, 5.0, 3.0, 1.0], dtype=np.float32)
    df = pd.DataFrame({"high": values + 1, "low": values - 1})
    ...
    values = np.array([3.0, 2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.9, 0.8], dtype=np.float32)
    df = pd.DataFrame({"high": values + 2, "low": values - 2})
    actual_labels = utils.create_critical_labels(df, thresh_entry=2.5, thresh_hold=1.5)
...
def test_create_critical2_labels():
    df_critical = pd.DataFrame(
        {
            "values": [1.0, 2.0, 3.0, 0.0, 2.0, 1.0, 5.0, 3.0, 1.0],
            "next_critical_values": [3, 3, 0, 5, 5, 5, 1, 1, np.nan],
            "uptrends": [True, True, False, True, True, True, False, False, True],
    ...
    df_critical = pd.DataFrame(
        {
            "values": [3.0, 2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.9, 0.8],
            "next_critical_values": [0, 0, 0, 2, 2, np.nan, np.nan, np.nan, np.nan],
...
def test_create_smadiff_labels():
    values = np.array([1.0, 2.0, 3.0, 0.0, 2.0, 1.0, -1, 0.0, 1.0])
    df = pd.DataFrame({"high": values + 1, "low": values - 1})
    actual_labels = utils.create_smadiff_labels(
        df, window_size_before=2, window_size_after=3, thresh_entry=1.0, thresh_hold=0.0
    )
...
def test_create_future_labels():
    values = np.array([1.0, 2.0, 3.0, 0.0, 2.0, 1.0, -1, 0.0, 1.0])
    df = pd.DataFrame({"high": values + 1, "low": values - 1})
    actual_labels = utils.create_future_labels(
        df, future_step_min=3, future_step_max=5, thresh_entry=1.5, thresh_hold=0.0
    )
...
def test_create_smatrend_labels():
    ...
    df = pd.DataFrame({"high": values + 1, "low": values - 1})
    actual_labels = utils.create_smatrend_labels(
        df, window_size=3, step_before=2, step_after=2, thresh_entry=4
    )
...
def test_create_gain_labels():
    ...
    future_sma = np.array(
        [25 / 3, 19 / 3, 12 / 3, np.nan, np.nan, np.nan, np.nan, np.nan]
    )
...
def test_create_dummy2_labels():
    df_x_dict = {
        "continuous": {"1min": pd.DataFrame({"sma10_frac_lag1": np.arange(0, 100, 10)})}
    }
...
def test_create_dummy3_labels():
    df_x_dict = {
        "sequential": {
            "1min": pd.DataFrame({"close": [0, 10, 20, 30, 30, 25, 15, 20, 0]})
        }
    }
